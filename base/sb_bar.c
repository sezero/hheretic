...
PATCH_REF PatchLTFACE;
PATCH_REF PatchRTFACE;
PATCH_REF PatchBARBACK;
PATCH_REF PatchCHAIN;
PATCH_REF PatchSTATBAR;
PATCH_REF PatchLIFEGEM;
//PATCH_REF PatchEMPWEAP;
//PATCH_REF PatchLIL4BOX;
PATCH_REF PatchLTFCTOP;
PATCH_REF PatchRTFCTOP;
//PATCH_REF PatchARMORBOX;
//PATCH_REF PatchARTIBOX;
PATCH_REF PatchSELECTBOX;
//PATCH_REF PatchKILLSPIC;
//PATCH_REF PatchMANAPIC;
//PATCH_REF PatchPOWERICN;
PATCH_REF PatchINVLFGEM1;
PATCH_REF PatchINVLFGEM2;
PATCH_REF PatchINVRTGEM1;
PATCH_REF PatchINVRTGEM2;
PATCH_REF PatchINumbers[10];
PATCH_REF PatchNEGATIVE;
PATCH_REF PatchSmNumbers[10];
PATCH_REF PatchBLACKSQ;
PATCH_REF PatchINVBAR;
PATCH_REF PatchARMCLEAR;
PATCH_REF PatchCHAINBACK;
//byte *ShadeTables;
extern byte *screen;
int FontBNumBase;
int spinbooklump;
...
void SB_Init(void)
{
	...
	{
		PatchLIFEGEM = WR_CacheLumpNum(W_GetNumForName("LIFEGEM0")
...
void SB_Drawer(void)
{
	...
	if(CPlayer->powers[pw_flight])
	{
			...
			|| !(CPlayer->powers[pw_flight]&16))
		{
			...
			if(CPlayer->mo->flags2&MF2_FLY)
			{
				...
				{
					V_DrawPatch(20, 17, WR_CacheLumpNum(spinflylump+15, PU_CACHE));
				}
				else
				{
					V_DrawPatch(20, 17, WR_CacheLumpNum(spinflylump+frame, PU_CACHE));
	...
	if(CPlayer->powers[pw_weaponlevel2] && !CPlayer->chickenTics)
	{
		...
		{
			frame = (leveltime/3)&15;
			V_DrawPatch(300, 17, WR_CacheLumpNum(spinbooklump+frame, PU_CACHE));
			BorderTopRefresh = true;
			UpdateState |= I_MESSAGES;
		}
		else
...
#endif
		V_DrawPatch(182, 161, WR_CacheLumpNum(W_GetNumForName("useartia") + ArtifactFlash - 1, PU_CACHE));
		ArtifactFlash--;
